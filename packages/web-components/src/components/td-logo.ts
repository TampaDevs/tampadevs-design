...
/**
 * Tampa Devs Logo Component
 * Official Tampa Devs logo with approved sizing and variants
 *
 * @element td-logo
 *
 * @prop {'full' | 'wordmark' | 'icon'} variant - Logo variant (icon only, wordmark only, or full)
 * @prop {'xs' | 'sm' | 'md' | 'lg' | 'xl' | '2xl'} size - Logo size
 * @prop {boolean} monochrome - Use single color (white) instead of brand colors
 * @prop {string} href - Optional link URL
 * @prop {string} textColor - Custom text color (CSS color value)
 * @prop {string} customText - Custom wordmark text (default: "Tampa Devs")
 *
 * @csspart logo - The logo container
 * @csspart icon - The icon element
 * @csspart text - The text element
 */
@customElement('td-logo')
export class TdLogo extends LitElement {
  static styles = [
    tokens,
    reset,
    css`
      ...
      .wordmark-devs {
        color: var(--logo-accent-color, var(--td-color-coral, #E85A4F));
      }

      :host([monochrome]) .wordmark-devs {
        color: var(--logo-text-color, white);
      }
  ...
  @property({ type: String, reflect: true }) size: LogoSize = 'md';
  @property({ type: Boolean, reflect: true }) monochrome = false;
  @property({ type: String }) href = '';
  @property({ type: String }) textColor = '';
  @property({ type: String }) customText = 'Tampa Devs';

  private renderIcon() {
    // Official Tampa Devs pirate logo SVG
    const redColor = this.monochrome ? 'currentColor' : '#d70100';
    const blackColor = this.monochrome ? 'currentColor' : '#000';

    return html`
      <div class="icon" part="icon">
        <svg viewBox="0 0 260 260" xmlns="http://www.w3.org/2000/svg" style="fill-rule:evenodd;clip-rule:evenodd;stroke-linejoin:round;stroke-miterlimit:2;">
          ${svg`
            ...
            <path d="M234.921,98.695c5.29,-5.291 8.112,-12.571 8.133,-20.801c-8.232,0.021 -15.512,2.843 -20.802,8.134c-4.108,4.108 -6.707,9.424 -7.682,15.444c0.039,0.333 0.097,0.661 0.133,0.996c0.158,1.484 0.024,2.948 -0.357,4.349c8.137,-0.071 15.332,-2.88 20.575,-8.122" style="fill:${redColor};fill-rule:nonzero;"/>
            <path d="M207.629,116.382c0.021,8.229 2.844,15.509 8.134,20.8c5.29,5.291 12.57,8.112 20.8,8.132c-0.021,-8.229 -2.843,-15.509 -8.133,-20.8c-5.291,-5.291 -12.571,-8.112 -20.801,-8.132" style="fill:${redColor};fill-rule:nonzero;"/>
  ...
  private renderWordmark() {
    // Split "Tampa Devs" into Tampa (white) and Devs (coral)
    // For custom text, try to split on space or just show as-is
    const text = this.customText;
    const spaceIndex = text.lastIndexOf(' ');

    if (spaceIndex > 0 && text.toLowerCase().includes('devs')) {
      const firstPart = text.substring(0, spaceIndex); // "Tampa" without trailing space
      const secondPart = text.substring(spaceIndex + 1); // "Devs"
      return html`
        <span class="wordmark" part="text">
          <span class="wordmark-tampa">${firstPart}</span> <span class="wordmark-devs">${secondPart}</span>
        </span>
      `;
    }

    // Default: show full text in primary color
    return html`
      <span class="wordmark" part="text">
        <span class="wordmark-tampa">${text}</span>
      </span>
    `;
  }
  ...
  render() {
    ...
    const textColorStyle = this.textColor ? `--logo-text-color: ${this.textColor}` : '';

    if (this.href) {
      return html`
        <a class="logo" href=${this.href} part="logo" style=${textColorStyle || nothing}>
          ${content}
        </a>
      `;
    }
    ...
    return html`
      <span class="logo" part="logo" style=${textColorStyle || nothing}>
        ${content}
      </span>
    `;
  }
